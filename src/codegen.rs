...
fn compile_call(ctx: &mut CodegenContext, call: &Call) -> CompileResult {
    ...
    let has_restarg_ptr = unsafe { ctx.builder.build_struct_gep(function_ptr, 7, "has_restarg_ptr") };
    ...
    let n_varargs = ctx.builder.build_int_sub(ctx.llvm_ctx.i64_type().const_int(args_count as u64, false), arity.into_int_value(), "n_varargs");
    ...
    ctx.builder.build_conditional_branch(has_restarg.into_int_value(), &vararg_block, &no_vararg_block);

    ctx.replace_cur_block(Rc::new(merge_vararg_block));

    let vararg_phi = ctx.builder.build_phi(ctx.lookup_known_type("unlisp_rt_object"), "phi");
